...
def generate_data(k, n, dimension, mu_list, sigma_list):
    ...
    for i in range(k):
        X[i*n : (i+1)*n, :dimension] = np.random.multivariate_normal(mu_list[i], sigma_list[i], size=n)
...
def log_likelihood(x, mu_list, sigma_list, pi_list):
    ...
    for i in range(x.shape[0]):
        ...
        for j in range(mu_list.shape[0]):
            pi_times_pdf_sum += pi_list[j] * multivariate_normal.pdf(x[j], mean=mu_list[j], cov=sigma_list[j])
...
def gmm(X, k, epsilon=1e-5):
    ...
    for times in range(k-1):
        temp_ans = []
        for i in range(x.shape[0]):
            temp_ans.append(np.sum([np.linalg.norm(x[i] - mu_list[j]) for j in range(len(mu_list))]))
    ...
    while True:
        ...
        if iterations % 10 == 0:
            log_l_list = log_l_list.append([{'Iterations':iterations, 'log likelihood':old_log_l}], ignore_index=True)
